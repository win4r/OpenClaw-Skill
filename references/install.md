...
openclaw doctor --fix    # Apply config migrations (e.g. Telegram, Firecrawl path changes)
openclaw gateway restart # Apply changes
```

### Windows: Verify `gateway.cmd` Version

On Windows, the `~/.openclaw/gateway.cmd` file (generated by `openclaw gateway install`) contains hardcoded version strings in comments and the `OPENCLAW_SERVICE_VERSION` environment variable. **npm/package updates do not automatically update these values.**

After updating, check the file:

```bash
cat ~/.openclaw/gateway.cmd | grep -i version
```

If the version is stale, either:
1. Run `openclaw gateway install` to regenerate the file, or
2. Manually update the version in `gateway.cmd` (two places):
   - Comment line: `rem OpenClaw Gateway (v<new-version>)`
   - Environment variable: `OPENCLAW_SERVICE_VERSION=<new-version>`
```
